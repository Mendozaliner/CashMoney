Updated: 2026-08-06 s30 (E45 CRQS-CycleOverlay DISCARDED. Howard Marks 4-component cycle tilt (CAPE 25%, VIX complacency 20%, 12m momentum 20%, ERP 15%; EPS excluded — CRQS handles it) fires only when cycle_score > threshold AND EPS gate inactive. 6 configs: cycle_threshold ∈ {0.70,0.75,0.80} × cycle_scale ∈ {0.80,0.90}. Best CycleOverlay(th=0.80,sc=0.90): mean_wf=1.034 (passes 1.033 bar), MaxDD=-16.02% (passes), DSR=1.000 (passes), CI=[+0.0413,+0.8520] (passes) — BUT End$1k=$11,982 FAILS the ≥$12,104 CRQS terminal-value bar. All 6 configs underperform CRQS champion in terminal value ($10,857–$11,982 vs $12,381). Root cause: tilt fires on only 6–16% of OOS days; modest equity reduction during "complacent bull" phases gives away more bull-market return than it protects in drawdowns (MaxDD identical to CRQS at -16.02%). The EPS quality gate in CRQS already handles late-cycle risk when earnings deteriorate; the cycle overlay is largely redundant. CRQS champion UNCHANGED. New file: strategies/crqs_cycle_overlay.py (for future Phase-3 reference). Howard Marks cycle score today: 0.665 (mid_late_cycle) — val=0.804, fear=0.781, mom=0.826, eps=0.0, erp=0.951. Score below 0.70 threshold: overlay would NOT fire even if adopted. Portfolio mark NEW ATH: SPY @ $769.79 (2026-08-05). 1.0001 SPY + $252.28 cash = $1,022.15 (+$12.19 / +1.207% since s29; SPY +1.600% same window; all-time +2.215% vs SPY +2.752% → CRQS -0.537% behind). Signal UNCHANGED: SPY=0.75, IEF=0, GLD=0, cash=0.25. IEF below SMA200 ($93.31 < $93.93); GLD below SMA200 ($389.64 < $411.72). No rebalance. Guardrails G1-G7 ALL GREEN. Live track: 0/3 completed months, first checkpoint ~2026-08-13. PHASE-2 GRADUATION CRITERION 1 MET (CI [+0.0534,+0.8629]); awaiting Criterion 2 (3 live months). NOTE: STATE.md s29 entry was stale — E43 Kelly-CRQS (vt=0.15 adopted, mean_wf=1.044) and E44 CRQS-MinVar (discarded, End$1k=$7,422 << CRQS $12,104) were FULLY RUN in s29 per hypotheses.jsonl, not architecture-only as previously noted. CRQS champion End$1k is now $12,381 (through 2026-08-05 data).)
...
**Latest mark (2026-08-05 close, session 30):**
1.0001 SPY × $769.79 = $769.87 + $252.28 cash (T-bill 3.725%/yr × 2d) = **$1,022.15**
(NEW ALL-TIME HIGH). +$12.19 / +1.207% from s29 $1,009.96. SPY same window: +1.600%.
CRQS behind SPY by 0.393% this window (75% exposure during rally). All-time: **+2.215%**
vs SPY +2.752% → CRQS −0.537% behind since inception.
CRQS signal UNCHANGED: SPY=0.75, IEF=0, GLD=0, cash=0.25. EPS quality gate still FIRED
(12m EPS growth negative). IEF $93.31 < SMA200 $93.93 → no IEF. GLD $389.64 < SMA200
$411.72 → no GLD. Inflation regime (corr=+0.489) but no defensive asset qualifies.
No rebalance needed. Guardrails G1–G7 ALL GREEN. Peak $1,022.15, DD from peak 0.0%.
...
Full-sample Sharpe 0.967, MaxDD -16.02%, End$1k $12,381 (through 2026-08-05), CI [+0.0534,+0.8629].
